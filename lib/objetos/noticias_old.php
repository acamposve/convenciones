...
//FUNCION UPDATE NOTICIAS
 	...
 	function update ( $id_noticia,$titulo,$fecha,$texto_completo,$texto_resumen,$categorias_noticias,$estatus_noticia )  {
		include( '../local/configuracion.php' );
		$dbconn = $LibSite->openDBConn(0);	
		$query = 	"UPDATE noticias SET resumen_noticia = '".$texto_resumen."', noticia_completa = '".$texto_completo."', ". 
					...
					"estatus_noticia = '".$estatus_noticia."' WHERE `noticias`.`codigo_noticia` =".$id_noticia;
...
function listarNoticias( $ini , $fin, $filtro) {
		...
		$list_sql  = "select * from noticias ".
					 "INNER JOIN categoria_noticias ON noticias.codigo_categoria = categoria_noticias.codigo_categoria ".
					 " WHERE titulo  LIKE '%".$filtro."%' order by nombre_categoria,titulo ASC LIMIT ".$ini.", ".$fin."";
...
function insertarNoticias ( $titulo,$fecha,$texto_completo,$texto_resumen,$categorias_noticias,$estatus_noticia )  {
		
		include( '../local/configuracion.php' );
		
		$dbconn = $LibSite->openDBConn(0);	
		
		$insert_sql = 	"INSERT INTO `noticias` (`resumen_noticia` ,`noticia_completa` ,`codigo_categoria` ,`titulo` ,`fecha_publicacion` ,`estatus_noticia` )".
						"VALUES ( '".$texto_resumen."', '".$texto_completo."', '".$categorias_noticias."', '".$titulo."', '".$fecha."','Pendiente')";
...
//FUNCION CONTAR CATEGORIAS DE NOTICIAS PARA PAGINADOR
